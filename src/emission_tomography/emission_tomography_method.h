...
void emissionTomographyMethod(const Array2D<T1> &gather,
    ...
    auto min_times_to_sources = std::make_unique<T2[]>(n_sources);
    ...
    for (std::ptrdiff_t bl_ir = 0; bl_ir < n_receivers; bl_ir += receivers_block_size) {
        ...
        const Array2D<T1> receivers_coords_block(const_cast<T1 *>(&receivers_coords(bl_ir, 0)), curr_receivers_block_size,
                                                3);
