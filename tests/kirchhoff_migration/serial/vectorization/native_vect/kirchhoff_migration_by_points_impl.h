...
//    alignas(sizeof(vector_data_t)) T1_without_cv res_tmp_arr[vector_dim_data];
    ...
    for (; i_p < n_points_without_remainder; i_p += vector_dim_data) {

        ForLoopResTmp<vector_dim_data - 1>::assign_res_tmp(point_processer, i_p, result_data);

//        simd_functions.storeu(result_data + i_p, simd_functions.add(simd_functions.loadu(res_tmp_arr),
//                                                                    simd_functions.loadu(result_data + i_p)));
    }
