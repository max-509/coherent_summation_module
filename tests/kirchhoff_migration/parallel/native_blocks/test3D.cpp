...
void run_program(CohSumType<double, double> coh_sum, test_data_generator3D<double> &data_gen,
	...
	for (std::ptrdiff_t rec_bl = 0; rec_bl < NxR*NyR; rec_bl += receivers_step) {
	    ...
	    if (is_trans) {
	        ...
	        res = perf_wrapper(std::bind(coh_sum, std::ref(gather), std::ref(times_to_source), std::ref(times_to_receivers), z_dim, y_dim, x_dim, dt, result_data, 120));
	    } else {
...
void test_n_sou_greater_n_smpls(std::ofstream &measurements_file) {
	...
	measurements_file << "by points without blocks;";
	...
	run_program(kirchhoffMigrationCHG3D<double, double>,
	        data_gen,
	        measurements_file,
	        x0_r, x1_r, NxR,
	        y0_r, y1_r, NyR,
...
int main(int argc, char const *argv[]) {
    ...
    std::ofstream measurements_file;
    create_measurements_file(std::string("./measurements3D_") + SIMD_EXTENSION + ".csv", measurements_file);
