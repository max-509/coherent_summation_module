#ifndef _KIRCHHOFF_MIGRATION_NATIVE_BLOCKS_H
#define _KIRCHHOFF_MIGRATION_NATIVE_BLOCKS_H
...
void kirchhoffMigrationCHG2DNativeBlocks(const Array2D<T1> &gather,
...
template<typename T1, typename T2>
void kirchhoffMigrationCHG3DNativeBlocks(const Array2D<T1> &gather,
                             const std::vector<T2> &times_to_source,
                             const Array2D<T2> &times_to_receivers,
                             std::ptrdiff_t z_dim, std::ptrdiff_t y_dim, std::ptrdiff_t x_dim,
                             double dt,
                             T1 *result_data,
                             std::ptrdiff_t p_block_size = 230) {
...
#endif //_KIRCHHOFF_MIGRATION_NATIVE_BLOCKS_H