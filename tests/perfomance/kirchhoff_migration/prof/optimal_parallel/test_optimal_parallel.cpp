...
void test() {
	...
	auto *result_data = new double[z_dim*y_dim*x_dim];
	...
	for (std::size_t i_r_y = 0; i_r_y < NyR; ++i_r_y) {
	    double y_r = y0_r + dy_r * static_cast<double>(i_r_y);
	...
	for (std::ptrdiff_t rec_bl = 0; rec_bl < NxR*NyR; rec_bl += receivers_step) {
	    kirchhoffMigrationCHG3D(gather, times_to_source, times_to_receivers, z_dim, y_dim, x_dim, dt, result_data, 60);
	}
