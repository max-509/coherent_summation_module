...
void run_program(const CohSumType<double, double>& coh_sum,
	...
	for (std::size_t rec_bl = 0; rec_bl < NxR; rec_bl += receivers_step) {
	    ...
	    if (is_trans) {
	        ...
	        res = perf_wrapper(std::bind(coh_sum, std::ref(gather), std::ref(times_to_source), std::ref(times_to_receivers), z_dim, x_dim, dt, result_data, 600));
	    } else {
...
void test_n_sou_greater_n_smpls(std::ofstream &measurements_file) {
	...
	measurements_file << "native by blocks;";
	...
	run_program(kirchhoffMigrationCHG2D<double, double>,
	        data_gen,
	        measurements_file,
	        x0_r, x1_r, NxR,
	        receivers_step,
